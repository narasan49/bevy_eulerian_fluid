...
@group(0) @binding(0) var w0: texture_storage_3d<r32float, write>;
@group(0) @binding(1) var w1: texture_storage_3d<r32float, read>;
@group(0) @binding(2) var w_solid: texture_storage_3d<r32float, read>;
...
fn solve_w(@builtin(global_invocation_id) gid: vec3u) {
    let factor = constants.dt / (constants.dx * constants.rho);
    let x = vec3i(gid);
    let dim = vec3i(textureDimensions(w0));
    if any(x >= dim) {
        return;
    }
    ...
    if (f.z == 0.0) {
        textureStore(w0, x, textureLoad(w_solid, x));
        return;
    }

    var p_plus = textureLoad(p0, x).r;
    var p_minus = textureLoad(p0, x - vec3i(0, 0, 1)).r;

    let level_plus = textureLoad(levelset_air0, x).r;
    let level_minus = textureLoad(levelset_air0, x - vec3i(0, 0, 1)).r;
    if (level_minus >= 0.0 && level_plus < 0.0) {
        ...
        textureStore(w0, x, vec4f(0.0));
        return;
    }

    let w = textureLoad(w1, x);
    let dw = vec4f(factor * (p_plus - p_minus), 0.0, 0.0, 0.0);
    textureStore(w0, x, w - dw);
}
