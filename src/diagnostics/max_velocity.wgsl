...
@group(0) @binding(3) var<storage, read_write> sum: f32;
...
fn partial_reduction(
    ...
    loop {
        if lid < stride {
            workgroup_sum[lid] = max(workgroup_sum[lid], workgroup_sum[lid + stride]);
        }
        if stride == 1 {
            break;
        }
...
fn reduction(
    ...
    for (var stride: u32 = WG_SIZE_2 / 2; stride > 0; stride /= 2) {
        if lid < stride {
            partials2[lid] = max(partials2[lid], partials2[lid + stride]);
        }
        workgroupBarrier();
    }
