...
// alive_particles_mask_scan must be padded with 0 to be a multiple of BLOCK_SIZE.
@group(0) @binding(0) var<storage, read> alive_particles_mask: array<u32>;
@group(0) @binding(1) var<storage, read_write> alive_particles_mask_scan: array<u32>;
...
var<workgroup> alive_particles_mask_scan_local: array<u32, BLOCK_SIZE>;
...
fn prefix_sum_per_workgroup(
    ...
    alive_particles_mask_scan_local[2 * idx] = alive_particles_mask[offset + 2 * idx];
    alive_particles_mask_scan_local[2 * idx + 1] = alive_particles_mask[offset + 2 * idx + 1];
    workgroupBarrier();
    ...
    for (var d = 0; d < depth; d++) {
        ...
        if (idx < width) {
            ...
            alive_particles_mask_scan_local[right_idx] += alive_particles_mask_scan_local[left_idx];
        }
    }
    ...
    if (local_invocation_index == 0) {
        ...
        sums[workgroup_idx] = alive_particles_mask_scan_local[last_idx];
        alive_particles_mask_scan_local[last_idx] = 0;
    }
    ...
    for (var d = 0; d < depth; d++) {
        ...
        if (idx < width) {
            ...
            let temp = alive_particles_mask_scan_local[left_idx];
            alive_particles_mask_scan_local[left_idx] = alive_particles_mask_scan_local[right_idx];
            alive_particles_mask_scan_local[right_idx] += temp;
        }
    }

    workgroupBarrier();
    alive_particles_mask_scan[offset + 2 * idx] = alive_particles_mask_scan_local[2 * idx];
    alive_particles_mask_scan[offset + 2 * idx + 1] = alive_particles_mask_scan_local[2 * idx + 1];
}
...
fn add_scanned_block_sums(
    ...
    alive_particles_mask_scan[idx] += sums[block_idx];
}
