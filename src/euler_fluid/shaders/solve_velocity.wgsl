...
@group(3) @binding(1) var levelset_air: texture_storage_2d<r32float, read_write>;
...
fn solve_velocity_u(@builtin(global_invocation_id) invocation_id: vec3<u32>) {
    ...
    let level_minus = textureLoad(levelset_air, x - vec2<i32>(1, 0)).r;
    let level_plus = textureLoad(levelset_air, x).r;
...
fn solve_velocity_v(@builtin(global_invocation_id) invocation_id: vec3<u32>) {
    ...
    let level_minus = textureLoad(levelset_air, x - vec2<i32>(0, 1)).r;
    let level_plus = textureLoad(levelset_air, x).r;
    if (level_minus >= 0.0 && level_plus >= 0.0) {
