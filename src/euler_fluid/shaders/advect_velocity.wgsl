...
@group(1) @binding(0) var levelset0: texture_storage_2d<r32float, read_write>;
@group(1) @binding(1) var levelset1: texture_storage_2d<r32float, read_write>;
...
fn advect_u(
    ...
    let levelset_x_u = levelset_at(levelset_solid, vec2<f32>(x_u) + vec2<f32>(0.0, 0.5));
    if (levelset_x_u <= 0.0) {
    ...
    if (backtraced_x_u.x < 0.0 || backtraced_x_u.x > dim_u.x - 1.0 || backtraced_x_u.y < 0.0 || backtraced_x_u.y > dim_u.y - 1.0) {
        ...
        let level = levelset_at(levelset_solid, backtraced_x_u + vec2<f32>(0.0, 0.5));
        if (level < 0.0) {
            let levelset_index = vec2<i32>(backtraced_x_u + vec2<f32>(0.0, 0.5));
            ...
            let corrected_backtraced_x_u = round(backtraced_x_u + level * level_gradient);
            let backtraced_u: f32 = u_at(u0, corrected_backtraced_x_u);
            if (backtraced_x_u.x < 0.0 || backtraced_x_u.x > dim_u.x - 1.0 || backtraced_x_u.y < 0.0 || backtraced_x_u.y > dim_u.y - 1.0) {
                textureStore(u1, x_u, vec4<f32>(0.0, 0.0, 0.0, 0.0));
            } else {
...
fn advect_v(
    ...
    if (backtraced_x.x < 0.0 || backtraced_x.x > dim_v.x - 1.0 || backtraced_x.y < 0.0 || backtraced_x.y > dim_v.y - 1.0) {
        ...
        if (level < 0.0) {
            ...
            var level_gradient = normalize(vec2<f32>(
                0.5 * (levelset_iplusj - levelset_ij + levelset_iplusjplus - levelset_ijplus),
                0.5 * (levelset_ijplus - levelset_ij + levelset_iplusjplus - levelset_iplusj),
            ));
            if (level_gradient.x != 0.0 || level_gradient.y != 0.0) {
