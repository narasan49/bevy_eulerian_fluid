...
fn extrapolate_v(
    ...
    if (is_valid == 1) {
        ...
        if all(right < dim) {
            let is_valid_right = textureLoad(in_is_v_valid, right).r;
            if is_valid_right == 1 {
                new_u += textureLoad(v0, right).r;
                count += 1;
            }
        }
        ...
        if all(left > vec2<i32>(0)) {
            let is_valid_left = textureLoad(in_is_v_valid, left).r;
            if is_valid_left == 1 {
                new_u += textureLoad(v0, left).r;
                count += 1;
            }
        }
        ...
        if all(top < dim) {
            let is_valid_top = textureLoad(in_is_v_valid, top).r;
            if is_valid_top == 1 {
                new_u += textureLoad(v0, top).r;
                count += 1;
            }
        }
        ...
        if all(bottom > vec2<i32>(0)) {
            let is_valid_bottom = textureLoad(in_is_v_valid, bottom).r;
            if is_valid_bottom == 1 {
                new_u += textureLoad(v0, bottom).r;
                count += 1;
            }
        }
        ...
        if count > 0 {
            new_u /= f32(count);
            textureStore(v0, idx, vec4<f32>(new_u, 0, 0, 0));
            textureStore(out_is_v_valid, idx, vec4<i32>(1, 0, 0, 0));
        } else {
            textureStore(out_is_v_valid, idx, vec4<i32>(0, 0, 0, 0));
        }
    }
}