...
#import bevy_fluid::coordinate::{interp2d_vertex, runge_kutta};
...
fn advect_levelset(
    ...
    let x = index_to_vertex_position(idx);
    ...
    let new_level = interp2d_vertex(levelset_air0, x_new);
    textureStore(levelset_air1, idx, vec4<f32>(new_level, 0.0, 0.0, 0.0));
}

fn index_to_vertex_position(
    x: vec2<i32>,
) -> vec2<f32> {
    return vec2<f32>(f32(x.x) - 0.5, f32(x.y) - 0.5);
}