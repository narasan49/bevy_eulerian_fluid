#import bevy_fluid::coordinate::{right, top};
...
@group(2) @binding(2) var levelset_solid: texture_storage_2d<r32float, read_write>; // nx + 1, ny + 1
...
fn divergence(@builtin(global_invocation_id) invocation_id: vec3<u32>) {
    ...
    let rhs = (f.iplusj * u_iplusj - f.iminusj * u_ij + f.ijplus * v_ijplus - f.ijminus * v_ij) 
        + ((1.0 - f.iplusj) * u_solid_iplusj - (1.0 - f.iminusj) * u_solid_ij + (1.0 - f.ijplus) * v_solid_ijplus - (1.0 - f.ijminus) * v_solid_ij);
